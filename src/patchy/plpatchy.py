...
def load_patches(filename, num_patches=0):
    j = 0
    Np = 0
    patches = [None for _ in range(num_patches)]
...
def load_particles(filename, patch_types, num_particles=0):
...
def export_interaction_matrix(patches, filename="interactions.txt"):
    with open(filename, 'w') as f:
        f.writelines(
            [
                f"patchy_eps[{p1.type()}][{p2.type()}] = 1.0\n"
...
class PLPatchyParticle:
    ...
    def export_to_lorenzian_mgl(self, patch_colors, particle_color, patch_width=0.1, patch_extension=0.2):
        # sout = '%f %f %f @ %f C[%s] ' % (self.cm_pos[0],self.cm_pos[1],self.cm_pos[2],self._radius,particle_color)
...
class PLPSimulation:
    def __init__(self, seed=69):
        ...
        self._particle_colors = ['blue', 'green', 'red', 'black', 'yellow', 'cyan', 'magenta', 'orange', 'violet'] * 100
        self._patch_colors = ['green', 'violet', 'pink', 'brown', 'orange', 'red', 'black'] * 100
    ...
    def load_input_file(self, input_file):
        ...
        self._patch_types = load_patches(patchy_file, self._N_patch_types)

        self._particle_types = load_particles(particle_file, self._patch_types, self._N_particle_types)
    ...
    def load_from_dps_files(self, input_file, topology_file, config_file, conf_to_skip=0):
    ...
    def export_to_mgl(self, filename, regime='w', icosahedron=True):
        ...
        for p in self.particles:
            ...
            if icosahedron:
                self.print_icosahedron(p, particle_color)

        out.write(sout)
        out.close()

    def print_icosahedron(self, p, particle_color):
        return "%f %f %f @ 0.5 C[%s] I %f %f %f %f %f %f \n" % (
            p.cm_pos[0], p.cm_pos[1], p.cm_pos[2], particle_color, p.a1[0], p.a1[1], p.a1[2], p.a2[0], p.a2[1],
            p.a2[2])

    def export_to_lorenzian_mgl(self, filename, regime='w', icosahedron=True):
        out = open(filename, regime)
        sout = ".Box:%f,%f,%f\n" % (self._box_size[0], self._box_size[1], self._box_size[2])
        for p in self.particles:
            patch_colors = [self._get_color(pat._color, True) for pat in p._patches]
    ...
    def export_to_francesco_mgl(self, filename, regime='w', icosahedron=True):
        out = open(filename, regime)
        sout = ".Box:%f,%f,%f\n" % (self._box_size[0], self._box_size[1], self._box_size[2])
        for p in self.particles:
            patch_colors = [self._get_color(pat._color, True) for pat in p._patches]
            particle_type = p._type
