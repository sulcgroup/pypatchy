...
class PatchyRunResult:
    ...
    def getClusterCategories(self, target_name, target_graph, verbose=False, parallel=False, sample_every=-1):
        ...
        if os.path.isfile(metadata_file):
            with open(metadata_file, 'rb') as f:
                metadata = pickle.load(f)
                if target_name not in metadata["categorized_timepoints"]:
        ...
        # otherwise construct empty dataframe, settle in with beverage
            ...
            cluster_categories = pd.DataFrame(
                columns=["nt", "temp", "duplicate", "tidx", "clustercategory", "sizeratio"])
        # log tidxs processed
        ...
        if not tidxs_processed >= set(range(0, self.num_timepoints(), sample_every)):
            if verbose:
                print(
                    f"Missing {len(set(range(0, self.num_timepoints(), sample_every)) - tidxs_processed)} timepoints... Categorizing...")
    ...
    def analyseClusterYield(self, target_name, target_graph, cutoff=1, overreach=False, verbose=False, parallel=False,
        ...
        if sample_every == -1:
            sample_every = get_sample_every()
        cluster_yields_file = self.get_path() + f"cluster_yields_{target_name}_C{cutoff}_O_{overreach}.pickle"
        if os.path.isfile(cluster_yields_file):
            ...
            if verbose:
                print(
                    f"Computing yields of {self.print_descriptors()} with target={target_name}, overreach={overreach}, cutoff={cutoff}...")
            # get parameters
            ...
            data = pd.DataFrame(index=pd.MultiIndex.from_product([
                [self.narrow_type_number], [self.temperature], [self.duplicate_number],
                np.arange(0, tlength, sample_every)
            ...
            if len(cluster_categories.index) > 0:
                ...
                if not overreach:
                    cluster_categories = cluster_categories.loc[
                        cluster_categories['clustercategory'].isin([ClusterCategory.SUBSET,
