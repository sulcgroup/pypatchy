...
def showClusterSizes(results, targets_for_reference):
    ...
    max_cluster_size = max(max_cluster_size, *[len(results.targets[target_name]['graph']) for target_name in targets_for_reference])
    ...
    for tidx, tdata in enumerate(data_by_temperature):
        c = [
                alt.Chart(tdata[tdata['i'] == cluster_idx], title="Assembly Yields at T=%f" % results.temperatures()[tidx]).mark_point().encode(
                    x=alt.X('time:Q', title="Time (megasteps)", axis=alt.Axis(tickCount=5)),
                    y=alt.Y(
                        "cluster_size:Q",
                        title="Cluster Size",
        ...
        for target_name in targets_for_reference:
            line = alt.Chart(pd.DataFrame({'cluster_size': [len(results.targets[target_name]['graph'])]})).mark_rule().encode(y='cluster_size')
