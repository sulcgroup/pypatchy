...
from ...util import BadSimulationDirException, simulation_run_dir, simulation_analysis_dir

if __name__ == '__main__':
    # set up argument parser
    argparse_descr = "Analyzes clusters.txt files produced by oxDNA's PLClusterTopology observable during execution " \
                     "of PatchShapeInteraction, producing pickled graph files. The graph files are in turn processed " \
                     "to produce yield data. "
    parser = argparse.ArgumentParser(prog="ComputeYields",
                                     description=argparse_descr)
    parser.add_argument("-p",
                        "--path",
                        action="store",
                        required=True,
                        help="The path at which to find the dataset to process")
    parser.add_argument("-t",
                        "--target",
                        action="store",
                        required=False,
                        default=None,
                        help="The target topology (will automatically run calculations for all targets if none)")
    parser.add_argument("-i",
                        "--interval",
                        action="store",
                        required=False,
                        type=int,
                        default=get_sample_every(),
                        help="The interval at which to sample the timepoints (default: every 10 timepoints)")
    # parse args
    ...
    if args.target is not None:
        ...
        runresult.getClusterCategories(args.target,
                                       dataset.targets[args.target]['graph'],
                                       verbose=True,
                                       parallel=False,
        ...
        for targetname in dataset.targets:
            ...
            runresult.getClusterCategories(targetname, dataset.targets[targetname]['graph'],
                                           verbose=True,
                                           parallel=False,
                                           sample_every=args.interval)
